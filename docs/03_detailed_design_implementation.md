...
| **シーケンス作成** | ルックバック長の窓で入力シーケンス X と対応する目的変数 y を生成。 | 入力: 説明変数 DataFrame, 目的変数列, lookback。出力: (X, y) の配列。 | 時点 $t$ の 1本先対数収益 $y_t$ に対して、その直前までの過去 $L$ 本 $[x_{t-L}, \ldots, x_{t-1}]$ を入力シーケンスとする（現在時点 $x_t$ は含めない）。デフォルト lookback=100。 |
...
| 項目 | 内容 |
|------|------|
| **モデル構築** | 入力: シーケンス形状 (samples, lookback, features)。出力: スカラー（1時点先の対数収益）。LSTM の層数・ユニット数・エポック数は PSO の探索対象。活性化・Dropout・L2 等は実装で固定またはパラメータ化。実装上は、学習安定性と損失スケールの扱いやすさのため、対数収益に一定倍率（例: `TARGET_SCALE = 1e4`）を掛けたスケール空間で学習を行い、評価・可視化の直前に 1/`TARGET_SCALE` を掛けて元スケールに戻す。 |
| **PSO 最適化** | 探索対象: (1) ニューロン数、(2) エポック数、(3) LSTM 隠れ層数。目的関数: 検証データでの RMSE を最小化。各粒子で LSTM を構築・学習し、検証 RMSE を返す。RMSE は `TARGET_SCALE` 倍された対数収益に対して計算されるが、定数倍であるためハイパーパラメータの順位付けは変わらない。 |
...
| 項目 | 内容 |
|------|------|
| **評価指標** | RMSE（二乗平均平方根誤差）、MAE（平均絶対誤差）、MAPE（%）（分母に `y_true + eps` を使用）、R2（決定係数）。compute_metrics には **元スケールの対数収益**（`TARGET_SCALE` で割り戻した値）を渡し、対数収益スケールで算出する。 |
| **評価の流れ** | テストデータで予測 → モデル出力（`TARGET_SCALE` 倍された対数収益）を 1/`TARGET_SCALE` で割り戻して元の対数収益スケールに復元 → 上記指標を算出・解釈。グラフ表示時は価格スケールに戻す（対数収益の場合は、例: `last_close * exp(pred_log_return)` で終値に復元してからプロット）。 |
