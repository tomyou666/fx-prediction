...
| 項目 | 内容 |
|------|------|
| **モデル構築** | 入力: シーケンス形状 (samples, lookback, features)。出力: スカラー（TARGET_HORIZON 本先の対数収益）。LSTM の層数・ユニット数・エポック数は PSO の探索対象。活性化・Dropout・L2 等は実装で固定またはパラメータ化。実装上は、学習安定性と損失スケールの扱いやすさのため、対数収益に一定倍率（例: `TARGET_SCALE = 1e4`）を掛けたスケール空間で学習を行い、評価・可視化の直前に 1/`TARGET_SCALE` を掛けて元スケールに戻す。 |
