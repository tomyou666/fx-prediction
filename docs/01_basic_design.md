...
- **定常性**: 対数収益・対数乖離に変換し、定常に近いデータのみをモデルに入力する。
  ...
  - **ボラティリティ・クラスタリング**: 説明変数・目的変数ともに EWMA を用いたローリング標準化を採用する。目的変数は「t 時点で観測可能な」過去の目的変数のみで標準化する（後述）。
  - **ブラックスワン**: ±3σ_ewma を超える値を 3σ_ewma でクリップしてから標準化する。
  - **取引所の曜日・時間**: 曜日と時間を sin/cos による周期エンコーディングで考慮する。
  - **サンプル重みづけ**: 標準化後の目的変数 $y'$ に対して $w_t = 1 + |y'_t|$ の線形式を適用し、絶対値が大きいほどペナルティがかかるように学習する。
...
- **目的変数の正規化**: 目的変数にも **指数加重移動平均（EWMA）による標準化** を適用する。説明変数と同様、**±3σ_ewma でクリップしてから** 標準化する。ただし t 時点では $Y_{t-\mathrm{TARGET\_HORIZON}}$ までしか観測できないため、t 時点での目的変数の標準化は **$Y_{t-h}$ から $Y_{t-(h+L)}$**（$h$ = TARGET_HORIZON、$L$ = LOOKBACK）の窓で μ・σ を算出し、$y_t$ をクリップしたうえで標準化する。予測値 $\hat{y}$ を元スケールに戻す際は、そのサンプルに対応する **t 時点の μ と σ** を用いて $\hat{y}_{\mathrm{orig}} = \hat{y}' \cdot \sigma_t + \mu_t$ とする。
- **サンプル重みづけ**: 標準化後の目的変数 $y'_t$ に対して **$w_t = 1 + |y'_t|$** を重みとして適用し、絶対値が大きいサンプルほど損失への寄与を大きくする（大きい値ほどペナルティがかかる）。
...
- **目的変数の EWMA 標準化（情報集合）**: t 時点では $y_{t-h}$ までしか観測されないため、t 時点での目的変数 $y_t$ の標準化に用いる μ・σ は、**観測可能な過去の目的変数** $Y_{t-h}, Y_{t-h-1}, \ldots, Y_{t-h-L+1}$（すなわち $Y_{t-h}$ から $Y_{t-(h+L)}$ の窓）のみから EWMA で算出する。説明変数と同様、**±3σ_ewma でクリップしてから** 標準化する。これにより未来情報のリークを防ぎ、外れ値の影響を有界にする。
- **逆変換**: 予測値 $\hat{y}'$（標準化スケール）を元の対数収益スケールに戻すときは、**そのサンプルが対応する時点 t の** $\mu_t$ と $\sigma_t$ を用いて $\hat{y} = \hat{y}' \cdot \sigma_t + \mu_t$ とする。
- **サンプル重み**: 標準化後の目的変数 $y'_t$ に対し $w_t = 1 + |y'_t|$ をサンプル重みとして学習に用いる。$|y'_t|$ が大きいほど重みが大きくなり、予測を外したときのペナルティが大きくなる。
