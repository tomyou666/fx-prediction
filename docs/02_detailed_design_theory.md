...
本節では、LSTM–PSO ハイブリッドモデルが為替時系列の $h$ 期先対数収益予測（$h$ は共通パラメータ TARGET_HORIZON、デフォルト 6）において有効である理由を、理論的に整理する。
...
$$
y_t = \ln\frac{P_{t+h}}{P_t}
$$

- $P_t$: 時点 $t$ の USD/JPY 終値
- $h$: **TARGET_HORIZON**（何本先で学習・推論するか。デフォルト 6）
- $y_t$: $h$ 期先の対数収益（リターン）
...
この定式化では、$h=1$ のとき $y_t$ は「$t$ 時点の情報までを用いた、$t+1$ 時点のリターン予測」に対応する。$h \geq 2$ のときは $h$ 本先までの複合リターンとなり、実運用ではホライズンに応じた解釈（例: 6 本先なら約 30 分先のリターン）となる。
...
説明変数と同様、目的変数（$h$ 期先対数収益 $y_t$）もボラティリティの時間変化の影響を受ける。一方で、t 時点で観測可能な目的変数は **$y_{t-h}$ まで**である（$y_t$ は $t+h$ 時点にならないと確定しない）。したがって、t 時点で $y_t$ を標準化する際の μ・σ は、**t 時点までに観測可能な目的変数の窓**、すなわち $Y_{t-h}, Y_{t-h-1}, \ldots, Y_{t-h-L+1}$（$Y_h$ から $Y_{t-(h+l)}$）のみから EWMA で算出する。目的変数も **±3σ_ewma でクリップしてから** 標準化し、外れ値の影響を有界にする。これにより、未来情報を使わない一貫した情報集合の下で標準化と逆変換が行われる。予測値 $\hat{y}'$ を元スケールに戻す際は、そのサンプルに対応する時点 t の $\mu_t$, $\sigma_t$ を用いて $\hat{y} = \hat{y}' \sigma_t + \mu_t$ とする。

---

#### (5'') サンプル重みづけ（大きい絶対値へのペナルティ）

標準化後の目的変数 $y'_t$ の絶対値が大きいサンプルは、相場の変動が大きい局面に対応する。これらを等しく扱うと、変動が小さい通常時の誤差と変動が大きい局面の誤差が同じ重みになり、運用上重要な「大きな値動き」の予測精度が相対的に軽視されうる。そこで、**サンプル重み**を $w_t = 1 + |y'_t|$ とし、$|y'_t|$ が大きいほど損失への寄与を大きくする。これにより、大きい値動きを外したときのペナルティが相対的に大きくなり、モデルが大きな変動をより重視して学習する。
...
| ボラティリティ・クラスタリング | 説明変数・目的変数ともに EWMA によるローリング標準化で局所スケールを吸収する | 時点 $t$ 以前の情報のみ使用。目的変数は $Y_{t-h}$ ～ $Y_{t-h-L+1}$ の窓で μ・σ を算出 |
| 極端な外れ値（厚い裾） | 説明変数・目的変数ともに ±3σ_ewma でクリップしてから標準化し、損失と分散推定のロバスト性を確保する | 外れ値の影響を有界化 |
| 目的変数の逆変換 | 予測値はそのサンプルに対応する時点 t の μ_t, σ_t で元スケールに戻す | 一貫した情報集合 |
| 大きな値動きの重視 | サンプル重み $w_t = 1 + |y'_t|$ で、絶対値が大きいほどペナルティを大きくする | 大きな変動の予測精度を相対的に重視 |
